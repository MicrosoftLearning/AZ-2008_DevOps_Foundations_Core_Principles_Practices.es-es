...
- Una suscripción de Azure en la que tenga al menos el acceso de nivel de colaborador. Si no tiene una suscripción de Azure, puede registrarse para obtener una [evaluación gratuita](https://azure.microsoft.com/free).
...
En este ejercicio, implementará IaC con CI/CD en aplicaciones web de Azure App Service con Acciones de GitHub y una plantilla de Bicep.
...
1. Cambie a la ventana del explorador que muestra la cuenta de GitHub y asegúrese de que todavía está autenticado (si no es así, inicie sesión con su cuenta de usuario de GitHub).
...
1. Abra el directorio **.github/workflows** y seleccione el archivo **eshoponweb-cicd.yml** para ver su contenido. Tenga en cuenta que este flujo de trabajo de Acciones de GitHub contiene los trabajos **buildandtest** y **deploy**, que constan de los siguientes pasos:
...
1. Tenga en cuenta que el flujo de trabajo se basa en un secreto para autenticarse en la suscripción de Azure de destino durante el paso Configurar la CLI de Azure, como se muestra en la instrucción `creds: ${{ secrets.AZURE_CREDENTIALS }}`.
...
1. En Azure Portal, en el cuadro de texto de búsqueda de la parte superior de la página, escriba **Grupos de recursos** y seleccione **Grupos de recursos** en la lista de resultados.
1. En la página **Grupos de recursos**, seleccione **+ Crear**.
1. En el cuadro de texto **Grupos de recursos**, escriba **rg-eshoponweb-westeurope**.
...
1. En la página **Grupos de recursos**, seleccione **+ Crear**.
1. En el cuadro de texto **Grupos de recursos**, escriba **rg-eshoponweb-eastus**.
...
1. Copie el valor del identificador de suscripción devuelto por el segundo comando y regístrelo. ya que lo necesitará más adelante en este ejercicio.
...
1. Copie toda la salida con formato JSON del comando y regístrela. Lo necesitará en breve. La salida debe tener el formato que se asemeja al siguiente texto:
...
1. En el panel **Secretos de acciones / Nuevo Secreto**, en el cuadro de texto **Nombre**, escriba **AZURE_CREDENTIALS**.
...
1. Vuelva al explorador web que muestra la sesión de Bash en el panel de Cloud Shell y ejecute el siguiente comando para generar el nombre de la primera aplicación web de App Service que va a implementar:

   ```cli
   echo devops-webapp-westeurope-$RANDOM$RANDOM
   ```

1. Copie el valor devuelto por el comando y anote el valor. Lo usará más adelante en este ejercicio.
1. En la sesión de Bash en el panel de Cloud Shell, ejecute el siguiente comando para generar el nombre de la segunda aplicación web de App Service que va a implementar:
...
1. Si se le pide que habilite Acciones de GitHub, seleccione **Entiendo mis flujos de trabajo, continúe y habilítelos**.
...
1. En la ventana del explorador web que muestra Azure Portal, en el cuadro de texto de búsqueda de la parte superior de la página, escriba **App Services** y seleccione **App Services** en la lista de resultados.
...
1. En la nueva pestaña del explorador, compruebe que se muestra la aplicación web y que es funcional. También puede comprobar la segunda aplicación web en la región **Este de EE. UU.** de la misma manera.
